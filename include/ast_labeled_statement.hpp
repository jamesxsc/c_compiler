...
namespace ast {
    ...
    public:
        ...
        LabeledStatement(StatementPtr statement, ConstantExpressionPtr caseExpr)
                : statement_(std::move(statement)), identifier_(std::nullopt), case_(std::move(caseExpr)),
                  defaultCase_(false) {};

        explicit LabeledStatement(StatementPtr statement)
                : statement_(std::move(statement)), identifier_(std::nullopt), case_(), defaultCase_(true) {};
