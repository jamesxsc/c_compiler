...
namespace ast {
    ...
    void ArrayIndexExpression::EmitRISC(std::ostream &stream, Context &context, Register destReg) const {
        ...
        Register addressTemp = useFloat ? context.AllocateTemporary() : destReg; // todo here and elsewhere, do a sweep for clobbering potential
        { Context::ScopedEmitLHS guard(context, !isPointerSyntax);
        ...
        if (useFloat)
            context.FreeRegister(addressTemp);
    }
